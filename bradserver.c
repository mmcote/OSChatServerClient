...
//error checks start here
	...
	if ((buffer[0] != '@') && (buffer[0] != '?')){
		printf("Invalid message type. Must be ? or @")'
		}
	...
	if (wrong? = 1)
		printf("Query request format incorrect. Must start with ?, followed by whiteboard entry number, followed by newline character.);
	}
	...
	while (*q){
		*q++;
		if isdigit(*q){
			//unfinished
	//check if format correct -update
